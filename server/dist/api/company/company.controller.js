...
export async function getCompany(req, res, next) {
    ...
    if (!isValid)
        throw new BadRequestError('Invalid company number');
...
export async function getBasicCompanyData(req, res, next) {
    const uuid = getUuidFromALS();
    const companyNumber = req.params.companyNumber;
    const isValid = companyNumberService.isValidCompanyNumber(companyNumber);
    if (!isValid)
...
export async function getCompanyEmployees(req, res, next) {
    const uuid = getUuidFromALS();
    const companyNumber = req.params.companyNumber;
    const isValid = companyNumberService.isValidCompanyNumber(companyNumber);
    if (!isValid)
...
export async function getCompanyEmployeeBasicTableData(req, res, next) {
    const uuid = getUuidFromALS();
    const companyNumber = req.params.companyNumber;
    const isValid = companyNumberService.isValidCompanyNumber(companyNumber);
    if (!isValid)
    ...
    const basicEmployeeData = await employeeService.getBasicEmployeeTableData(companyEmployeeIds);
    logger.info(`companyController - getCompanyEmployeeBasicTableData, companyId: ${company._id}`);
    res.status(200).json({
        success: true,
...
export async function getCompanyEmployeeAdvancedTableData(req, res, next) {
    const uuid = getUuidFromALS();
    const companyNumber = req.params.companyNumber;
    const isValid = companyNumberService.isValidCompanyNumber(companyNumber);
    if (!isValid)
    ...
    const companyEmployeeIds = company.employees;
    const advancedEmployeeData = await employeeService.getAdvancedEmployeeTableData(companyEmployeeIds);
    logger.info(`companyController - getCompanyEmployeeAdvancedTableData, companyId: ${company._id}`);
    res.status(200).json({
        success: true,
...
export async function getCompanyEmployeesAccounts(req, res, next) {
    const uuid = getUuidFromALS();
    const companyNumber = req.params.companyNumber;
    const isValid = companyNumberService.isValidCompanyNumber(companyNumber);
    if (!isValid)
...
export async function updateCompanyEmployeeAccount(req, res, next) {
    const uuid = getUuidFromALS();
    const companyNumber = req.params.companyNumber;
    const isValid = companyNumberService.isValidCompanyNumber(companyNumber);
    if (!isValid)
    ...
    const isValidEmployeeNumber = employeeNumberService.isValidEmployeeNumber(employeeNumber);
    ...
    const employeeDoc = await employeeService.getEmployeeDoc(companyNumber, employeeNumber);
    if (!employeeDoc) {
        res.status(404).json({
            success: false,
    ...
    const updatedAccount = await accountService.updateAccount(accountDoc._id, role, status);
    if (!updatedAccount)
        throw new BadRequestError('Account not updated');
    res.status(200).json({
        success: true,
...
export async function addEmployee(req, res, next) {
    ...
    const departmentDoc = await companyService.getCompanyDepartmentDocByName(company._id, departmentName);
    if (!departmentDoc) {
        res.status(404).json({
            success: false,
    ...
    // create employee
    const employee = await employeeService.createEmployee(profile._id, company._id, departmentDoc._id, position);
    if (!employee) {
        res.status(404).json({
            success: false,
...
export async function updateEmployee(req, res, next) {
    ...
    if (!employeeDoc) {
        ...
        const departmentDoc = await companyService.getCompanyDepartmentDocByName(companyDoc._id, departmentName);
        if (!departmentDoc)
            throw new BadRequestError('Cannot find department');
        const employee = await employeeService.createEmployee(profile._id, companyDoc._id, departmentDoc._id, position);
        if (!employee)
    ...
    if (employeeDoc) {
        const [updatedProfileData, updateEmployeeData, updatedCompanyData, updatedDepartmentData,] = accountService.sortAccountData(data);
        const updatedProfile = await profileService.updateProfile(employeeDoc.profile, updatedProfileData);
        const updatedEmployee = await employeeService.updateEmployee(employeeDoc._id, updateEmployeeData);
        const departmentDoc = await companyService.getCompanyDepartmentDocByName(employeeDoc.company, departmentName);
        if (!departmentDoc) {
            res.status(404).json({
                success: false,
                message: 'Department not found',
...
export async function deleteEmployee(req, res, next) {
    const uuid = getUuidFromALS();
    const companyNumber = req.params.companyNumber;
    const isValidCompanyNumber = companyNumberService.isValidCompanyNumber(companyNumber);
    ...
    if (accountDoc)
        await accountService.disconnectEmployee(accountDoc._id);
    // employee has no account
    if (!accountDoc)
        await profileService.deleteProfile(employeeDoc.profile);
...
export async function generateEmployeeNumber(req, res, next) {
    const uuid = getUuidFromALS();
    const companyNumber = req.params.companyNumber;
    const isValidCompanyNumber = companyNumberService.isValidCompanyNumber(companyNumber);
    if (!isValidCompanyNumber)
    ...
    const employeeNumber = await employeeNumberService.generateEmployeeNumber(companyDoc._id);
    res.status(200).json({
        success: true,
